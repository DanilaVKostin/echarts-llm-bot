def build_system_prompt(
    ...
    *,
    ...
    base = (
        ...
        "- ALWAYS explicitly set `itemStyle: { color: \"#...\" }` on EVERY series when first "
        "generating a chart. Never rely on ECharts default palette rotation — if colors are not "
        "pinned, asking to change one color will shift all others. "
        "For line charts also set `lineStyle: { color: \"#...\" }` to match `itemStyle.color`. "
        "Use this default palette (cycle through it): "
        "#5470c6, #91cc75, #fac858, #ee6666, #73c0de, #3ba272, #fc8452, #9a60b4, #ea7ccc.\n"
        "- When the user asks to change one series color, only modify that series — keep all "
        "other `itemStyle.color` and `lineStyle.color` values unchanged.\n"
